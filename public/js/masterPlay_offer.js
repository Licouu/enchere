...
let element = document.getElementsByClassName("duree_beat");
const audio = new Audio();

console.log(element[0]);
console.log(element[0].id);
audio.src=element[0].id;
audio.onloadedmetadata = function() {
            let durationInSeconds = Math.round(audio.duration);
            console.log(durationInSeconds);
            if (durationInSeconds > 60) {
                let minutes = Math.floor(durationInSeconds / 60);
                let seconds = durationInSeconds % 60;
                element[0].innerHTML=minutes + " minutes " + seconds + " seconds";
                console.log(minutes + " minutes " + seconds + " seconds");
            }else{
                element[0].innerHTML=durationInSeconds + " seconds";
            }
        }
...
Array.from(document.getElementsByClassName('playListPlay')).forEach((element)=>{
    element.addEventListener('click', (e)=>{
        ...
        if (e.target instanceof HTMLImageElement) {
            ...
            url=e.target.parentNode.parentNode.parentNode.querySelector('#beat_info');
        }
        else{
            music_url= e.target.id;
            image = e.target.parentNode.querySelector('img').src;
            url=e.target.parentNode.parentNode.querySelector('#beat_info');
        }
        console.log(image);
        ...
        */
        ...
        music.play();
        title.innerHTML =url.querySelector('h1').textContent+"<br><div class=\"subtitle\">"+url.querySelector('li').textContent+"</div>";
